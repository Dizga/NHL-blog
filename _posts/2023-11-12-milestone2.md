...
Certaine caractéristiques catégorielles comme `Type` et `Previous event type` on été encodé par encodage moyen cible. D'autres, tels que (`Team`, `Opp Team`, etc.) on été encodé par encodage ordinal car d'autres caractéristiques (`Team goals`, `Opp concedes`, etc.) encode déjà leur relation avec le label.
